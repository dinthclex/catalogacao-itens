 ...
 * no perfil). Ver progresso-sessao.md pra limitações documentadas.
 *
...
// de `register()`), perdendo o terracota do vaso (ver limitação #1 da
// RODADA 66/67 em progresso-sessao.md — agora corrigida PRA ESTE tipo).
...
function gerarTudo() {
  ...
  const objDir = path.resolve(__dirname, '..', '..', 'modelos', 'obj');
  fs.mkdirSync(objDir, { recursive: true });
  const malhaOutDir = path.resolve(__dirname, '..', '..', 'modelos');
