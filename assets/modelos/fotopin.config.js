/* assets/modelos/fotopin.config.js
 * NOVO (12/09/2026) — Modelo padrão do TIPO "pino de foto" (o retângulo
 * texturizado com a foto vinculada, ver engine3d.js "fotos vinculadas ao
 * mapa" / js/ambientephotos.js). Mesmo espírito de camera.config.js — ver
 * comentário grande lá e no topo de js/objectassets.js.
 *
 * DECISÃO DE ESCOPO: "fotopin" não é um `tipo` de `map.objects` (é um
 * registro `AmbientePhoto` do banco, ver comentário de escopo em
 * js/sceneobjects.js) — por isso ganha sua PRÓPRIA chave fixa 'fotopin'
 * (como 'camera'), em vez de cair no `_generic` por `tipo`. */
window.ObjectAssets.registerModel('fotopin', {
  id: 'fotopin',
  nome: 'Pino de foto',
  propriedades: { interativo: true },
  onModelSpawn(entity, ctx) {
    // roda 1x quando ESTE objeto aparece pela primeira vez na cena 3D
    //console.log('Objeto Pino de foto (id='+entity.id+') criado na cena.');
  },
  onModelClick(entity, ctx) {
    // roda ao clicar com o botão esquerdo NESTE objeto
    //console.log('Objeto Pino de foto (id='+entity.id+') clicado.');
    ctx.view3d._showFotoPinCard3D(entity);
  },
