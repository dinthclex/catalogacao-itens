/* assets/modelos/teto-modular.config.js
 * [13/09/2026] NOVO — pedido do usuário: "teto modular (escritórios)".
 * Segue o MESMO padrão de arquivo de override por tipo já usado por
 * coluna.config.js/gabinete.config.js: por enquanto o clique continua caindo
 * no cartão genérico (ctx.view3d._showObjectCard3D(entity)) — a malha de
 * verdade (placa fina com textura procedural de grade 60×60cm, branco-gelo)
 * é montada em js/engine3d.js (`_buildOneObjectMesh`, ramo genérico de
 * retângulo, checando `obj.tipo === 'teto-modular'`) e o perfil-base
 * (dimensões/cor de fallback) vive em js/engine3d-profiles.js
 * (`OBJECT3D_PROFILES['teto-modular']`). Customize aqui, sem mexer em
 * js/view3d.js, quando "Teto modular" precisar de comportamento próprio
 * (ex.: destacar visualmente ao clicar). */
window.ObjectAssets.registerModel('teto-modular', {
  ...
  onModelSpawn(entity, ctx) {
    ...
    //console.log('Objeto Teto modular (id='+entity.id+') criado na cena.');
  },
  onModelClick(entity, ctx) {
    // roda ao clicar com o botão esquerdo NESTE objeto
    //console.log('Objeto Teto modular (id='+entity.id+') clicado.');
