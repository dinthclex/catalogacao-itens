/* assets/modelos/monitor2.config.js
 * NOVO (13/09/2026) — VARIANTE INDEPENDENTE do tipo "monitor", mesmo
 * pedido/padrão de gabinete2.config.js (ver comentário grande lá): o
 * usuário pediu um SEGUNDO modelo de Monitor como objeto INDEPENDENTE no
 * catálogo, sem substituir o "monitor" já existente. Perfil 3D em
 * js/engine3d-profiles.js (OBJECT3D_PROFILES.monitor2 — bem mais fino que
 * a variante 1, estilo "monitor LED moderno" em vez de LCD/CRT grosso). */
window.ObjectAssets.registerModel('monitor2', {
  ...
  onModelSpawn(entity, ctx) {
    ...
    //console.log('Objeto Monitor (LED) (id='+entity.id+') criado na cena.');
  },
  onModelClick(entity, ctx) {
    // roda ao clicar com o botão esquerdo NESTE objeto
    //console.log('Objeto Monitor (LED) (id='+entity.id+') clicado.');
    ctx.view3d._showObjectCard3D(entity);
  },

  /* [13/09/2026] NOVO — "MODO COMPUTADOR" — mesmo mecanismo/motivo de
   * `monitor.config.js` (ver comentário grande lá): troca o botão padrão
   * "💻 Abrir aplicativo..." por "🖥️ Acessar este computador". */
