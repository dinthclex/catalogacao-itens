/* assets/modelos/robo.config.js
 * NOVO (13/09/2026) — tipo BASE do catálogo de robôs (infraestrutura
 * reutilizável pros 3 tipos concretos do backlog do usuário: limpeza,
 * copa/café, recepcionista). Comportamento de clique idêntico ao genérico
 * (cartão "🔧 Modelar em 3D"/"Fechar"), mesmo padrão de gabinete.config.js —
 * o que importa neste tipo é o PERFIL 3D (js/engine3d-profiles.js,
 * OBJECT3D_PROFILES.robo: cilindro baixo, ~40cm de diâmetro x ~30cm de
 * altura, cor neutra) mais o SISTEMA DE TRAJETO por Script (ver
 * assets/modelos/_exemplo-script-robo-trajeto.txt) — não um comportamento
 * de clique especial.
 *
 * Como usar (colocar um robô no mapa e dar trajeto a ele):
 *  1. Ferramentas → Objetos → escolha "robo" (ou uma das 3 variantes
 *     robo-limpeza/robo-copa/robo-recepcionista) e toque no mapa 2D.
 *  2. Abra o painel do objeto recém-criado → "➕ Adicionar Componente" →
 *     "Script".
 *  3. Cole o conteúdo de um dos arquivos de exemplo
 *     (_exemplo-script-robo-trajeto.txt genérico, ou um dos 3
 *     especializados) no campo de código do Script.
 *  4. Ajuste a lista de waypoints (comentada no topo do script) pras
 *     coordenadas reais do mapa do usuário (mesmas unidades de obj.x/
 *     obj.y — metros, mesmo sistema de coordenadas do mapa 2D).
 *  5. Abra a Visualização 3D — o robô anda pelo trajeto sozinho a cada
 *     quadro (Update(dt) do Script, mesmo mecanismo já usado pelos
 *     relógios/mesas animados do prédio). */
window.ObjectAssets.registerModel('robo', {
  ...
  onModelSpawn(entity, ctx) {
    ...
    //console.log('Objeto Robô (id='+entity.id+') criado na cena.');
  },
  onModelClick(entity, ctx) {
    // roda ao clicar com o botão esquerdo NESTE objeto
    //console.log('Objeto Robô (id='+entity.id+') clicado.');
