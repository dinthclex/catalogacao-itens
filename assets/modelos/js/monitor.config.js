/* assets/modelos/monitor.config.js
 ...
 * js/view3d.js, quando "Monitor" precisar de comportamento próprio. */
window.ObjectAssets.registerModel('monitor', {
  ...
  onModelSpawn(entity, ctx) {
    ...
    //console.log('Objeto Monitor (id='+entity.id+') criado na cena.');
  },
  onModelClick(entity, ctx) {
    // roda ao clicar com o botão esquerdo NESTE objeto
    //console.log('Objeto Monitor (id='+entity.id+') clicado.');
    ctx.view3d._showObjectCard3D(entity);
  },

  /* [13/09/2026] NOVO — "MODO COMPUTADOR" (pedido verbatim: "Ao clicar do pc,
   * não deve ser 'Abrir aplicativo: Monitoramento de Robôs' [...] mas sim
   * 'Acessar este computador'"). Usa a API `onModelCardButtons(entity, ctx)`
   * (ver comentário grande em js/objectassets.js e o exemplo em
   * `assets/modelos/_exemplo-model-botoes-customizados.txt`) pra REMOVER o
   * botão padrão `'robo-app'` (o antigo "💻 Abrir aplicativo...", decidido em
   * `_showObjectCard3DBody`, js/view3d.js) e pôr no lugar dele "🖥️ Acessar
   * este computador", que entra no MODO COMPUTADOR de verdade
   * (`View3D._acessarComputador3D` — câmera anima até ficar de frente ao
   * monitor, cursor normal aparece, área de trabalho com ícone/janela do app
   * de monitoramento — ver comentário grande da seção "MODO COMPUTADOR" em
   * js/view3d.js). */
  onModelCardButtons(entity, ctx) {
    return {
      ...
      adicionar: [
        {
          label: '🖥️ Acessar este computador',
          title: 'Usa a tela deste computador — abre a área de trabalho em frente ao monitor',
          onClick(entity, ctx) {
            ctx.view3d._acessarComputador3D(entity);
          },
        },
      ],
    };
