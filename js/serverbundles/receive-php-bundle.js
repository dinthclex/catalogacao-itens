/**
 * receive-php-bundle.js — NOVO (07/09/2026), pedido verbatim do usuario:
 * "Sim, atualize tudo. Na verdade, retire as copias embutidas do
 * js/settings.js e deixe em arquivos separados em .js (arquivo com apenas
 * uma variavel com uma string de texto gigante) mesmo para funcionar em
 * 'file:///'. E deixar mais organizado e estruturado."
 *
 * ANTES desta rodada, o conteudo de receive.php (pro botao "Baixar arquivos
 * do servidor (.php)" em Configuracoes) vinha como uma STRING GIGANTE
 * embutida DENTRO de js/settings.js (`_downloadServerBundle`) -- alem de
 * deixar aquele arquivo enorme/dificil de navegar, essa copia ja estava
 * DESATUALIZADA em relacao ao server/receive.php real (nem tinha a
 * funcionalidade de pastaDados/config.json de rodadas anteriores, quanto
 * mais a reorganizacao storage/ desta rodada) -- flagado ao usuario na
 * rodada anterior como pendencia separada; agora corrigido.
 *
 * Este arquivo faz duas coisas ao mesmo tempo: (1) organiza melhor o
 * projeto, tirando uma string de ~250 linhas de dentro de settings.js; (2)
 * ATUALIZA o conteudo pra ficar IGUAL ao server/receive.php real desta
 * mesma rodada (reorganizacao storage/3d,images,text,raw,app + migracao
 * automatica) -- gerado programaticamente a partir do arquivo real (nunca
 * digitado a mao), pra eliminar o risco de as duas copias divergirem de
 * novo por erro de transcricao.
 *
 * Carregado via <script src="js/serverbundles/receive-php-bundle.js">
 * ANTES de js/settings.js (ver index.html) -- funciona normalmente em
 * 'file:///' (script local comum, sem fetch/import nenhum, mesma forma que
 * qualquer outro arquivo de js/ ja e carregado). js/settings.js usa a
 * variavel global `RECEIVE_PHP_BUNDLE` abaixo em vez de ter o texto embutido
 * ali dentro.
 *
 * IMPORTANTE (risco documentado do projeto): todo backslash, crase (`) e
 * "${" do codigo PHP original abaixo estao ESCAPADOS (\\, \` e \${)
 * porque este arquivo inteiro e um template literal do JavaScript -- sem
 * isso, o proprio motor JS interpretaria essas sequencias (ex.: "\r\n"
 * viraria uma quebra de linha de verdade em vez do texto literal "\r\n"
 * que o PHP downloadado precisa ter). Gerado por escape automatico
 * (backslash -> crase -> "${", nesta ordem) a partir do arquivo real -- NAO
 * edite este arquivo a mao; edite server/receive.php e gere de novo.
 */
window.RECEIVE_PHP_BUNDLE = `<?php
