...
class Engine3D {
  ...
  _initThree(THREE) {
    ...
    // (`mesh.visible=false`) e um InstancedMesh irmão (`this._instancedPools[tipo]`)
    // passa a desenhá-la, com a MESMA posição/rotação (`mesh.userData._inst.matrix`,
  ...
  _rebuildInstancedPools() {
    ...
    grupos.forEach((meshes, tipo) => {
      ...
      // As instâncias ficam espalhadas por posições arbitrárias do mapa
      // inteiro — a caixa/esfera de corte automática do InstancedMesh (que o
      // three.js calcula só a partir da geometria de UMA instância, na
      // origem) cortaria a malha errado sempre que a câmera olhasse pra longe
      // da origem. Mesmo espírito de nunca depender de frustum culling
      // "de fábrica" pra objetos posicionados livremente.
      ...
      meshes.forEach((m, i) => {
        ...
        m.userData._inst = { tipo, index: i, matrix: m.matrix.clone() };
      ...
      this._instancedPools[tipo] = { mesh: inst, count: meshes.length };
    });
  }
  ...
  _syncInstanceVisibility(mesh) {
    ...
    const pool = this._instancedPools[inst.tipo];
    if (!pool) return;
  ...
  _buildOneObjectMesh(obj, wireframe, colWireframe) {
    ...
    if (!wireframe && this.mode !== 'hibrido' && obj.forma !== 'retangulo' && obj.forma !== 'poligono') {
